...
class App extends $HelperTrait {
  ...
  REDIS_USED = super.isValidVal(super.getEnvApp("REDIS_USED"), null, "true");
  ...
  APP_SERVER() {
    ...
    return super.isValidVal(APP_ENV, null, "development", "equal") ? `http://localhost:${APP_PORT}` : `https://${APP_DOMAIN}:${APP_PORT}`;
  }
  ...
  DB_SERVER() {
    ...
    if (DB_TYPE === "nosql") {
      return super.isValidVal(MONGODB_URL) ? MONGODB_URL : `${DB_DRIVER}://${DB_HOST}:${DB_PORT}/${DB_NAME}`;
    }

    if (DB_TYPE === "sql") {
      return super.isValidVal(DATABASE_URL) ? DATABASE_URL : `${DB_DRIVER}://${DB_USER}:${DB_PASS}@${DB_HOST}:${DB_PORT}/${DB_NAME}`;
    }
  }
}
